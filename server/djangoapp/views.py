...
def add_review(request, dealer_id):
    if request.method == 'POST':
        ...
        if data['purchase']:
            car = CarModel.objects.filter(id=request.POST['car_model']).first()
            data['purchase_date'] = datetime.strptime(request.POST['purchase_date'], '%Y-%m-%d').isoformat()
            data['car_model'] = car.name
            data['car_make'] = car.car_maker.name
            data['car_year'] = car.year.strftime("%Y")
